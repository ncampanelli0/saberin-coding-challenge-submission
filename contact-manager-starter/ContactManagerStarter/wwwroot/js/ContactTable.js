...
$(function () {
    ...
    $(document).on("click", "#addNewAddress", function () {
        ...
        if (validateAddress(street1, street2, city, state, zip)) {
            $("#addressList").append(
    ...
    $(document).on("click", "#saveContactButton", function () {
        function getEmailAddresses() {
            ...
            //todo implement check for text in textboxes
            ...
            let state = $('#newAddressState').val();
            let zip = $('#newAddressZip').val();
            var check = checkEmptyBoxesAddress(street1, street2, city, state, zip);
            if (check) {
    ...
    function validateAddress(newAddressStreet1, newAddressStreet2, newAddressCity, newAddressState, newAddressZip) {
        if ((newAddressStreet1 == "" || newAddressStreet2 == "") && newAddressCity == "" && newAddressState == "" && newAddressZip == "") {
    ...
    function checkEmptyBoxesAddress(street1, street2, city, state, zip)
    {
        if (!(street1 == "" && street2 == "" && city == "" && state == "" && zip == "") && (street1 == "" || street2 == "" || city == "" || state == "" || zip == ""))
        {
         
            alert("warning: address is filled in but you did not save it by pressing add");
            return false;
        }
        else
        {
            //alert("address is not filled in");
            return true;
        }
    }


    function checkEmptyBoxesEmail(email)
    {
        if (email == "") {

            alert("warning: email is filled in but you did not save it by pressing add");
            return false;
        }
        else {
            //alert("address is not filled in");
